...
export class ProcessWagerTransactionUseCase {
  ...
  async execute(
    ...
    const payloadHash = hashWagerCommand(command);
    const eventContext: EventContext = context ?? {
      correlationId: `${command.providerId}:${command.idempotencyKey}`,
    };
    try {
      return await this.transactions.execute((scope) =>
        this.process(scope, command, money, payloadHash, eventContext),
      );
    } catch (error) {
      ...
      if (isUniqueConstraint(error, 'wager_transactions_provider_external_unique')) {
        throw new ExternalTransactionConflictError();
      }
      if (isUniqueConstraint(error, 'wager_transactions_processed_reversal_unique')) {
        return this.transactions.execute((scope) =>
          this.persistDuplicateReversal(scope, command, money, payloadHash, eventContext),
        );
      }
      throw error;
    }
  ...
  async executeInScope(
    ...
    return this.process(
      ...
      hashWagerCommand(command),
      context,
    );
  }

  private async persistDuplicateReversal(
    scope: FinancialTransactionScope,
    command: ProcessWagerTransactionCommand,
    money: Money,
    payloadHash: string,
    ...
    const duplicateReversal = new InvalidReferenceError(
      'The reference was already reversed.',
      FailureCode.ReferenceAlreadyReversed,
    );

    return this.reject(scope, transaction, wallet.balance, duplicateReversal, context);
  }
  private async process(
    ...
    const replay = await scope.transactions.findByProviderAndIdempotencyKey(
      command.providerId,
    ...
    const external = await scope.transactions.findByProviderAndExternalTransactionId(
      command.providerId,
      command.externalTransactionId,
    );
    if (external !== undefined) {
      const idempotent = await scope.transactions.findByProviderAndIdempotencyKey(
        command.providerId,
        command.idempotencyKey,
      );
      if (idempotent !== undefined) {
        return this.replay(idempotent, payloadHash);
      }
    ...
    const afterLock = await scope.transactions.findByProviderAndIdempotencyKey(
      command.providerId,
      command.idempotencyKey,
    );
    if (afterLock !== undefined) {
      return this.replay(afterLock, payloadHash);
    }
    ...
    if (wallet.currency !== money.currency) {
      return this.reject(
        ...
        transaction,
        wallet.balance,
        new CurrencyMismatchError(wallet.currency, money.currency),
        context,
      );
    }
    try {
      ...
      if (command.referenceExternalTransactionId) {
        reference = await scope.transactions.findByProviderAndExternalTransactionId(
          command.providerId,
        ...
        if (reference === undefined) {
          ...
          transaction.scheduleReferenceRetry(
            0,
            this.pendingReference.firstAttemptAt(this.now()),
          );
        ...
        if (
          ...
          const referenceAlreadyReversedBySameKind =
      ...
      if (
        ...
        assertReversalDoesNotOverdraw(wallet, money);
      }

      const movement =
        direction === LedgerDirection.Debit
          ? wallet.debit(money, this.now())
          : wallet.credit(money, this.now());
      ...
      if (error instanceof DomainRuleViolationError) {
        return this.reject(scope, transaction, wallet.balance, error, context);
      }
  ...
  private async reject(
    ...
    await scope.outbox.append(
      this.events.rejected(tx, balance.toString(), balance.currency, context),
    );
  ...
  private replay(
    ...
    if (!tx.matchesPayload(payloadHash)) {
      throw new IdempotencyConflictError();
    }

    return this.result(tx, true);
  }
  ...
  private result(
    ...
    if (tx.resultBalance === undefined) {
      throw new Error('Persisted transaction lacks result balance.');
    }
...
export function canonicalizeWagerCommand(c: ProcessWagerTransactionCommand): string {
  return JSON.stringify({
    ...
    referenceExternalTransactionId: c.referenceExternalTransactionId ?? null,
    roundId: c.roundId,
    walletId: c.walletId,
  });
}
